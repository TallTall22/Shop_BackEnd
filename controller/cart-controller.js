...
const cartController = {
  ...
  createCart: async (req, res, next) => {
    ...
    try {
      ...
      if (order) {
        ...
        if (cart) {
          return res.json({ status: 'success', message: '你已經將此商品加入購物車' });
        }
  ...
  deleteCart: (req, res, next) => {
    ...
    Cart.findByPk(cartId)
      .then(cart => {
        if (!cart) {
          throw new Error('The Cart is not Existed');
        }
        return cart.destroy();
      })
    ...
    Promise.all([
      ...
      .then(([cart, product]) => {
        ...
        if (quantity > product.quantity) {
          return res.json({ status: 'success', message: '庫存已見底啦!!!' });
        }
        if (quantity < 1) {
          return res.json({ status: 'success', message: '不能再減啦!!!' });
        }
  ...
  linePayOrder: async (req, res, next) => {
    ...
    try {
      ...
      const headers = cartController.createSignature(uri, linePayBody);
